...
func (f *FileStream) Close() error {
	...
	if file, ok := f.tmpFile.(*os.File); ok {
		...
		if err2 != nil {
			err2 = fmt.Errorf("failed to remove tmpFile [%s]: %w", file.Name(), err2)
		} else {
			f.tmpFile = nil
		}
	}
...
func (f *FileStream) cache(maxCacheSize int64) (model.File, error) {
	if maxCacheSize > int64(configs.MaxBufferLimit) {
	...
	var buf []byte
	if configs.MmapThreshold > 0 && bufSize >= int64(configs.MmapThreshold) {
